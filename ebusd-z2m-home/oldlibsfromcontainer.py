...
def Heating_main(temp_diff):
    ...
    water_times = [[datetime.strptime('5:00', "%H:%M").time(),datetime.strptime('10:30', "%H:%M").time()], [datetime.strptime('16:00', "%H:%M").time(), datetime.strptime('20:00', "%H:%M").time()]]
    PIEC_OFF = 1
    settemp = 40
    podiff = {-0.25: 45, -0.5:50, -2:60}
    jodiff = {-0.5: 43, -0.7: 47, -1: 55}

    if is_time_in_range(water_times[0][0], water_times[0][1]) or is_time_in_range(water_times[1][0], water_times[1][1]):
        print(f'wlaczam wode')
        WODAOFF = 0
    else:
        print(f'wylaczam wode')
        WODAOFF = 1
